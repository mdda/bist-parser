...
class ArcHybridLSTM:
    ...
    def __evaluate(self, stack, buf, train):
        if len(stack) > 0:
            ...
            print stack.roots[-1].lstms
            print "empty: ", self.empty
        else:
            print "len stack: ", len(stack)
            print self.empty
            print self.empty.dim()
        #exit()    
        ...
        print topStack[0][0].dim()#.get_dimension()
        print topBuffer[0][0].dim()#.get_dimension()
       # exit()
    ...
    def Init(self):
        ...
        print concatenate(filter(None, [paddingWordVec, paddingPosVec, evec])).dim()
        print self.word2lstm.expr().dim()
        #exit()
        paddingVec = tanh(self.word2lstm.expr() * concatenate(filter(None, [paddingWordVec, paddingPosVec, evec])) + self.word2lstmbias.expr() )
        print paddingVec.dim()
        #exit()
        print self.nnvecs
        if self.nnvecs == 1:
            print "nnvecs equals 1"
            print paddingVec.dim()
        else:
            print "paddingVec = ", self.nnvecs
            print concatenate([paddingVec for _ in xrange(self.nnvecs)]).dim()
